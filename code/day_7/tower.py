...
def inspect_tower(string):
    programm_calls = string.split("\n")
    programms_carrying = [p.split(" -> ") for p in programm_calls if r"->" in p]   
    carrier = []
    carried_ones = []
    for programm in programms_carrying:
        carrier.append(re.sub(" \([0-9]+\)", "",programm[0])) 
        carried_ones.extend(programm[1].split(", "))
    for carry in carrier:
        if not carry in carried_ones:
            return carry
...
bcadp (91)
tmzxl (23)"""
print(inspect_tower(puzzle_input))
